...
#GOAL: Use the Bureau of Economic Analysis (BEA) API to get Real GDP data, 
...
def main():
	...
	df_cols = ["Year", "State", "Real GDP", "Unit"]
	rows = []
	...
	for child in root.iter('Data'):
	    ...
	    realgdp = child.attrib.get("DataValue")
	    realgdp2 = float(re.sub('[,]', '', realgdp))
	    
	    unit = child.attrib.get("CL_UNIT")
	    
	    rows.append({"Year": year, "State": state, 
	                 "Real GDP": realgdp2, "Unit": unit})
	...
	export_csv = results.to_csv('BEA_State_RealGDP_2000-2016_clean.csv', index = None, header=True)
...
if __name__ == '__main__':
    ...
    print("Calling realgdp_cleaning.py -> main() ")
    main()