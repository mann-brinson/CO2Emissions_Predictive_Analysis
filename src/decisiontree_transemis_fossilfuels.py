...
def main():
    ...
    emissions = pd.read_csv('EIA_State_CO2Emissions_all_sectors_1980-2016_clean.csv')
    ...
    def get_1516_changes(statecode_list, new_df1):
        ...
        for statecode in statecode_list:
            ...
            for index, rows in emis_2015.iterrows(): 
                ...
                my_list =[rows.CO2_Emissions_transportation, rows.CLTCB, rows.NGTCB, rows.PMTCB] 
    ...
    changes_1516_df.columns = ['D_Emis_transport','D_Coal_Cons', 'D_NatGas_Cons', 'D_Petrol_Cons']

    #Get the target classifier (D_Emis_transport) in a list
    #Take D_Emis_transport and turn into a list
    Y = changes_1516_df['D_Emis_transport'].tolist()

    #Get the attributes (D_Coal_Cons, D_NatGas_Cons, D_Petrol_Cons) in a list of lists
    X = []
    for index, row in changes_1516_df.iterrows():
        mylist = []
        mylist.append(row['D_Coal_Cons'])
        mylist.append(row['D_NatGas_Cons'])
        mylist.append(row['D_Petrol_Cons'])
        X.append(mylist)
    ...
    dot_data = export_graphviz(clf, out_file=None,  
                               ...
                               feature_names=['D_Coal_Cons', 'D_NatGas_Cons', 'D_Petrol_Cons'],
                               class_names=['Decrease', 'No Change', 'Increase'])

    graph = pydotplus.graph_from_dot_data(dot_data)  
    graph.write_png("decisiontree_transemis_fossilfuels_A.png")
    ...
    changes_allyears_dict = get_yr2yr1_changes(startend_years_list, statecode_list, new_df1)

    #Convert the dict to a pandas df
    changes_allyears_df = pd.DataFrame.from_dict(changes_allyears_dict)
    changes_allyears_df = changes_allyears_df.transpose()
    changes_allyears_df.columns = ['D_Emis_transport','D_Coal_Cons', 'D_NatGas_Cons', 'D_Petrol_Cons']

    #Output the dataframe to a csv file
    export_csv = changes_allyears_df.to_csv ('decisiontree_transemis_fossilfuels_B_data.csv', index = 1, header=True) 

    #Get the target classifier (D_Emis_transport) in a list
    #Take D_Emis_transport and turn into a list
    Y = changes_allyears_df['D_Emis_transport'].tolist()
    ...
    graph = pydotplus.graph_from_dot_data(dot_data)  
    graph.write_png("decisiontree_transemis_fossilfuels_B.png")
    ...
    # The training dataset will contain years 1980-2015. 
    ...
    year_list_8015 = list(set(new_df1['Years'].tolist()))
    year_list_8015.remove(2016)

    startend_years8015_list = []
    for i in range(len(year_list_8015)-1):
        year1 = year_list_8015[i]
        year2 = year_list_8015[i+1]
        startend_years = []
        startend_years.append(year1)
        startend_years.append(year2)
        startend_years8015_list.append(startend_years)

    #Training dataset: Gets changes from 1990-2015
    train_dict = get_yr2yr1_changes(startend_years8015_list, statecode_list, new_df1)
    ...
    #STEP 4: Compare the Y_pred with Y_actual, to get a true positive rate 
    print("Transportation Emisissions vs Fossil Fuels Decision Tree Accuracy:",metrics.accuracy_score(Y_actual, Y_pred))
...
if __name__ == '__main__':
    ...
    print("Calling decisiontree_transemis_fossilfuels.py -> main() ")
    main()